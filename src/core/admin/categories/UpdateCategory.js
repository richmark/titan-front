...
const Categories = ({ match }) => {
  ...
  const init = () => {
    getCategory(match.params.categoryId).then((oData) => {
      if (oData.error) {
        ...
        const { _id, name, category_image } = oData.data;
        setValues({
          _id: _id,
          name: name,
  ...
  const { _id, name, category_image, formData } = values;
  ...
  return (
    <DashboardLayout name="Product Management" detail={[<a href='/admin/categories'>Categories</a>, " / Update Category"]}>
      {showUpdateCategory()}
      {redirectPage()}
    </DashboardLayout>
  );
