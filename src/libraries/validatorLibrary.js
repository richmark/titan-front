...
export const oValidatorLibrary = () => {
    var oRules = {
        validators: {
            ...
            password: {
                message: 'Password must be minimum of six characters, at least one letter and one number',
                rule: (oVal, oParams, oValidator) => {
                    return oValidator.helpers.testRegex(oVal, /^(?=.*[A-Za-z])(?=.*\d)[A-Za-z\d]{6,}$/) && oParams.indexOf(oVal) === -1
                }
            }
        }
    };
